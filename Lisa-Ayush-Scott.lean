...
theorem H_normalizes_U :
  ...
  rw [toralSubgroup]
  refine (Subgroup.closure_le (K := Subgroup.normalizer
    ((unipotentSubgroup (c := c) : Subgroup (G (c := c))) : Set (G (c := c))))).2 ?_
  intro g hg
...
variable {I G : Type*} [Group G]

/--
If `H` is a perfect subgroup of `G`, viewed as an abstract group,
then every element of `H` lies in the commutator subgroup of `G`.
-/
lemma subgroup_le_commutator_of_perfect
    (H : Subgroup G)
    (hH : commutator H = ⊤) :
    H ≤ commutator G := by
  intro x hx

  -- Regard `x` as an element of the subgroup `H`.
  let y : H := ⟨x, hx⟩

  -- Since `H` is perfect, `y` lies in the commutator subgroup of `H`.
  have hy : y ∈ commutator H := by
    rw [hH]
    trivial

  -- Map this fact into the ambient group `G`.
  have hmap : x ∈ Subgroup.map H.subtype (commutator H) := by
    exact ⟨y, hy, rfl⟩

  -- The image of the commutator subgroup of `H` is the subgroup
  -- commutator `⁅H, H⁆` inside `G`.
  rw [Subgroup.map_subtype_commutator H] at hmap

  -- Finally, `⁅H, H⁆ ≤ ⁅⊤, ⊤⁆ = commutator G`.
...
/--
If a group `G` is generated by a family of perfect subgroups `S i`,
then `G` is perfect.

The countability assumption is included to match the mathematical theorem,
but it is not used in the proof.
-/
theorem perfect_of_generated_by_perfect_subgroups
    [Countable I]
    (S : I → Subgroup G)
    (hgen : (⨆ i : I, S i) = ⊤)
    (hperf : ∀ i : I, commutator (S i) = ⊤) :
    commutator G = ⊤ := by
  apply top_unique
  rw [← hgen]
  exact iSup_le fun i =>
    subgroup_le_commutator_of_perfect (S i) (hperf i)
